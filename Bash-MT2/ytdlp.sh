...
options=("YouTube" "Twitch" "Toggle Automation" "Delete Videos" "Backup" "Restore" "Update" "Quit")
...
do
    case $opt in
        "1080p")
export PS3=$'\033[0;31mSelect an option: \e[0m'
options=("Popular" "Recent" "Random" "Go Back")
select opt in "${options[@]}"
do
    case $opt in
            "Popular")
...
do
    case $opt in
            ...
            ~/.local/bin/ytdlp --match-filter '!was_live & original_url!*=/shorts/' -a ~/Configs/YTDLP/youtube-channel-sorted-popular.txt --config-locations ~/Configs/YTDLP/youtube-video-480p.conf --lazy-playlist --max-downloads 5 --sponsorblock-remove sponsor,selfpromo --sleep-interval 60 --max-sleep-interval 120 --js-runtimes deno:/home/soders/.deno/bin/deno --download-archive ~/Configs/YTDLP/watch-history.txt
            ~/Scripts/delete-empty-media-directories.sh && ~/Scripts/delete-empty-media-directories-2.sh
            ~/Scripts/completion-chime.sh && sleep 1
            fi
            fi
...
do
    case $opt in
        "1080p")
            ...
            ;;
        "480p")
            ~/.local/bin/ytdlp --match-filter '!is_live' -a ~/Configs/YTDLP/twitch-following-channel-urls.txt --config-locations ~/Configs/YTDLP/twitch-video-480p.conf --playlist-end 1 --lazy-playlist --dateafter now-7days --js-runtimes deno:/home/soders/.deno/bin/deno --download-archive ~/Configs/YTDLP/watch-history.txt
            ~/Scripts/delete-empty-media-directories.sh && ~/Scripts/delete-empty-media-directories-2.sh
            ~/Scripts/completion-chime.sh && sleep 1
            break
            ;;
...
do
    case $opt in
        "1080p")
            ...
            ;;
        "480p")
            ~/.local/bin/ytdlp --match-filter '!is_live' -a ~/Configs/YTDLP/twitch-bedtime-channel-urls.txt --config-locations ~/Configs/YTDLP/twitch-video-480p.conf --playlist-end 1 --lazy-playlist --dateafter now-7days --js-runtimes deno:/home/soders/.deno/bin/deno --max-downloads 1 --download-archive ~/Configs/YTDLP/watch-history.txt
            ~/Scripts/delete-empty-media-directories.sh && ~/Scripts/delete-empty-media-directories-2.sh
            ~/Scripts/completion-chime.sh && sleep 1
            break
            ;;
...
do
    case $opt in
        ...
        "480p")
            read -p "$(echo -e ${RED}"Enter VOD URLs: "${NC})" URLS
            ~/.local/bin/ytdlp $URLS --config-locations ~/Configs/YTDLP/twitch-video-480p.conf --js-runtimes deno:/home/soders/.deno/bin/deno
            ~/Scripts/delete-empty-media-directories.sh && ~/Scripts/delete-empty-media-directories-2.sh
            ~/Scripts/completion-chime.sh && sleep 1
            break
            ;;
