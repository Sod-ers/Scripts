...
~/.local/bin/ytdlp --match-filter "!was_live & original_url!*=/shorts/" $music_subscriptions --config-locations ~/Configs/YTDLP/video-480p.conf --playlist-end 1 --lazy-playlist --dateafter now-3days > /dev/null 2>&1&