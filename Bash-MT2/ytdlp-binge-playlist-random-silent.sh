...
~/.local/bin/ytdlp --match-filter '!was_live & original_url!*=/shorts/' $binge_playlist_url --config-locations ~/Configs/YTDLP/youtube-video-1080p.conf --playlist-random --max-downloads 5 --sleep-interval 60 --max-sleep-interval 120 --sponsorblock-remove sponsor,selfpromo --js-runtimes deno:/home/soders/.deno/bin/deno --download-archive ~/Configs/YTDLP/watch-history.txt > /dev/null 2>&1&
