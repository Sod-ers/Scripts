...
~/.local/bin/ytdlp --sleep-interval 60 --max-sleep-interval 120 --match-filter '!is_live' -a ~/Configs/YTDLP/twitch-following-channel-urls.txt --config-locations ~/Configs/YTDLP/twitch-video-1080p.conf --playlist-end 1 --lazy-playlist --dateafter now-3days --js-runtimes deno:/home/soders/.deno/bin/deno --download-archive ~/Configs/YTDLP/watch-history.txt > /dev/null 2>&1&
