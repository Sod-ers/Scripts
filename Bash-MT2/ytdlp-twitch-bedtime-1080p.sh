...
~/.local/bin/ytdlp --match-filter '!is_live' -a ~/Configs/YTDLP/twitch-bedtime-channel-urls.txt --config-locations ~/Configs/YTDLP/twitch-video-1080p.conf --playlist-end 1 --lazy-playlist --dateafter now-3days --js-runtimes deno:/home/soders/.deno/bin/deno --max-downloads 1 --download-archive ~/Configs/YTDLP/watch-history.txt > /dev/null 2>&1&
